...
`config/` のファイルは、配列を return するだけの PHP ファイルです。ファイル名とキーをつないで、`config('estat.app_id')` のように読めます。

これを設定すると `tinker` でも参照することができます。
設定することで、laravelのどこからでも簡単に呼び出すことができて非常に便利になります。
...
このコマンドは、`.env` を読み込んで config を組み立て、その結果を1つのファイルに書き出します。書き出したあとの起動では、そのファイルだけが読まれ、`.env` は読み込まれません。

一覧をリロードしても、表示は変わりません。`config()` が、書き出された値を返しているためです。`env()` との差は tinker で見えます。
...
`config/estat.php` の中の `env('ESTAT_APP_ID')` は、書き出すときに実行済みです。その結果がファイルに入っているので、`config()` は値を返します。
